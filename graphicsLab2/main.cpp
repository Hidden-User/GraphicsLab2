...
int main() {
	...
	F4 pCol = { 1.0f, 0.0f, 0.0f, 1.0f };
	F4 bf4;
	bf4.x = 0.0f;
	bf4.y = 0.0f;
	bf4.z = 0.0001f;
	bf4.w = 1.0f;
	viewArrow = { DirectX::XM_PI, 330.0f };

	LineList* lineList = new LineList();
	lineList->setDX(device, context);
	lineList->setLinesCount(184u);
	Line* line = lineList->getLine(0u);
	line->buildFromPointAndVector(bf4, viewArrow, pCol);

	pCol = { 0.0f, 0.0f, 1.0f, 1.0f };

	float _PiD2Buff = DirectX::XM_PIDIV2;
	float _PiBuff = 0.0f;
	F2 bf2;

	for (unsigned t = 1; t < 181; t++) {
		_PiD2Buff += oneAngleInRad;
		bf4.x = (float)cos(_PiBuff) * 350.0f;
		bf4.y = (float)sin(_PiBuff) * 350.0f;
		bf2.x = _PiD2Buff;
		bf2.y = 6.981228f;
		_PiBuff += oneAngleInRad;
		line = lineList->getLine(t);
		line->buildFromPointAndVector(bf4, bf2, pCol);
	}

	_PiBuff = DirectX::XM_PIDIV4;
	bf4.x = (float)cos(_PiBuff) * 340.0f;
	bf4.y = (float)sin(_PiBuff) * 340.0f;
	bf2.x = _PiBuff;
	bf2.y = 20.0f;
	line = lineList->getLine(181u);
	line->buildFromPointAndVector(bf4, bf2, pCol);
	bf4.x *= -1.0f;
	bf2.x = _PiBuff * 3.0f;
	line = lineList->getLine(183u);
	line->buildFromPointAndVector(bf4, bf2, pCol);
	bf4.x = 0.0f;
	bf4.y = 340.0f;
	bf2.x = _PiBuff * 2.0f;
	line = lineList->getLine(182u);
	line->buildFromPointAndVector(bf4, bf2, pCol);

	pCol = { 1.0f, 0.0f, 0.0f, 1.0f };

	_PiBuff = DirectX::XM_PI;

	line = lineList->getLine(0u);
	...
	while (active)
	{
		...
		if (viewArrow.x > _PiBuff) {
			viewArrow.x = _PiBuff;
		}
		if (viewArrow.x < 0.0f) {
			viewArrow.x = 0.0f;
		}
		line->setFromAToVec(viewArrow);
		lineList->update();
		lineList->draw();
